...
| Trino | `nifi` by default | `TRINO_INGESTION_PASSWORD` | Dev-only table operations in `raw`, `curated`, and `documents`; authenticates like every other identity since §4.2 |
...
### 4.2 Trino authentication

Until this was added, Trino had no authenticator at all: it believed whatever
username a client asserted. Anything that could reach the port could send
`X-Trino-User: psu-admin` and be treated as an administrator, which is the
single reason every port had to stay on loopback and why the stack could not
be pointed at real data.

Trino now requires a password, over TLS. Both arrive together because password
authentication is refused over plain HTTP:

- The identities and their passwords come from `.env`, hashed with bcrypt into
  a password file in the ignored runtime directory by the `trino-password-setup`
  one-shot service. No plaintext password is written anywhere in the stack.
- The certificate is self-signed, generated by `trino-tls-setup` with the
  `keytool` already present in the Trino image. Clients pass `--insecure` (or
  the equivalent) because there is no chain to verify against — the transport
  is unverified, the identity is not. A real certificate is perimeter work.
- The host port serves HTTPS only. The plain HTTP port stays inside the Compose
  network for the discovery URI and refuses authenticated requests, so it is
  not a way around the authenticator.
- Superset authenticates under its own name and then impersonates the signed-in
  user. Impersonation is allowed for that identity alone
  ([`config/opa/trino.rego`](config/opa/trino.rego)), so a leaked analyst
  credential cannot become an administrator one.

Verified by `./scripts/test-trino-auth.sh`: an asserted username with no
credential gets 401, the plain HTTP port refuses the same request, a wrong
password is rejected, one identity cannot log in with another's password, and
a normal identity attempting to impersonate is denied.

Authentication is not authorization. OPA still decides what each identity may
do (§6.3); what changed is that the username is now something a client proves
rather than something it claims. This is a development identity store, not PSU
SSO — SSO replaces the password file, it does not extend it.
...
| Trino | <https://localhost:8086> | Operator-only query status; HTTPS with a self-signed certificate, and a password is required |
...
| value of `TRINO_MAINTENANCE_USERNAME` | `psu_maintenance` | Compaction, snapshot expiry and orphan-file removal on `polaris`; deliberately cannot read any column |
...
### 6.10 Iceberg table maintenance

```bash
./scripts/run-maintenance.sh                    # compact and expire snapshots
./scripts/run-maintenance.sh --dry-run          # print the statements only
./scripts/run-maintenance.sh --include-orphans  # also delete orphaned files
./scripts/test-maintenance.sh                   # assert the identity boundary
```

What to maintain and how long to retain comes from `ingest.source_table`
(`optimize_enabled`, `expire_snapshots_days`, `orphan_retention_days`), and
every action is recorded in `ingest.maintenance_run`. A registered table that
has never actually been loaded is left out, so a skip is not reported as a
failure.

**Compaction is the performance half. Deletion is the other half, and it is a
PDPA obligation, not housekeeping.** Dropping an Iceberg table in this stack
does not remove its data files: an orphaned `published/enrollment_summary-*`
prefix left behind by an earlier drop is what proved it. Until orphan removal
has run, a deletion request is not actually satisfied — the rows are still
sitting in object storage, readable by anything with the storage credentials.
Measured on the synthetic data: `raw/` went from 226 objects to 90 with every
row still present.

`remove_orphan_files` deletes real files, so it is opt-in per invocation rather
than part of the default run, and the 7-day retention floor is left in place.
`--min-retention` lowers that floor for testing and prints a warning; it is for
synthetic data.

Maintenance runs as its own Trino identity (`TRINO_MAINTENANCE_USERNAME`,
group `psu_maintenance`). It can compact, expire and delete, and it cannot read
a single value of the data it maintains — `SELECT employee_id FROM
polaris.raw.sim_hr_employee` returns Access Denied for it. Trino asks the
authorizer for `SelectFromColumns` with an empty column list as part of
